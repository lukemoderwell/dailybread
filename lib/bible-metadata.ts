// Bible metadata: verse counts for each chapter
// Data source: Standard Protestant Bible canon

// Map book names to API.Bible book IDs (all 66 books)
...
// Format: { book: [chapter1_verses, chapter2_verses, ...] }
...
export function getNextBook(currentBook: string): string | null {
  const index = BIBLE_BOOKS.findIndex(book => book === currentBook);
  if (index === -1 || index === BIBLE_BOOKS.length - 1) return null;
  return BIBLE_BOOKS[index + 1];
}

export function getTotalChapters(book: string): number | null {
  const chapters = VERSE_COUNTS[book];
  return chapters ? chapters.length : null;
}
...
// Calculate the ending position after reading N verses from a starting position
export function calculateEndingPosition(
  start: VersePosition,
  versesToRead: number
): VersePosition | null {
  let { book, chapter, verse } = start;
  let remaining = versesToRead;

  while (remaining > 0) {
    const chapterVerseCount = getChapterVerseCount(book, chapter);
    if (!chapterVerseCount) return null;

    const versesLeftInChapter = chapterVerseCount - verse + 1;

    if (remaining <= versesLeftInChapter) {
      // We can complete reading within this chapter
      return {
        book,
        chapter,
        verse: verse + remaining - 1,
      };
    }
